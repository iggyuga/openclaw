...
import { computePositionSizeByATR } from "./strategy.js";
...
export async function checkStopsAndTakeProfit(): Promise<void> {
  ...
  for (const pos of positions) {
    ...
    if (pnlPct <= -config.strategy.stopLossPct) {
      log.warn(
        `STOP LOSS triggered for ${pos.symbol}: ${(pnlPct * 100).toFixed(2)}% (entry $${pos.entryPrice} → $${ticker.price})`,
      );
...
export async function handleSignal(symbol: string, signal: Signal): Promise<void> {
  ...
  if (direction === "BUY" && !hasPosition(symbol)) {
    ...
    if (pos) {
      await executeExit(symbol, pos.qty, "signal");
    }
  }
}
...
async function executeBuy(symbol: string, signal: Signal): Promise<void> {
  ...
  const fee = config.strategy.useLimitOrders
    ? config.strategy.makerFeePct
    : config.strategy.takerFeePct;

  // ATR-based position sizing: volatility-adjusted so we risk the same USD regardless of how wild the asset is
  const atr = signal.indicators.atr;
  const price = signal.indicators.price;
  const baseRisk = computePositionSizeByATR(usdAvailable, atr, price, config.strategy.riskPerTrade);
  // Scale by signal strength and subtract fees
  const riskAmount = baseRisk * signal.strength * (1 - fee);
  ...
  log.info(
    `Entry: $${riskAmount.toFixed(2)} | stop: -${(config.strategy.stopLossPct * 100).toFixed(1)}% ($${(fillPrice * (1 - config.strategy.stopLossPct)).toFixed(2)}) | tp: +${(config.strategy.takeProfitPct * 100).toFixed(1)}% ($${(fillPrice * (1 + config.strategy.takeProfitPct)).toFixed(2)}) | fee: ${(fee * 100).toFixed(2)}%`,
  );

  if (config.bot.dryRun) {
    log.trade(
      `[DRY RUN] ${config.strategy.useLimitOrders ? "LIMIT" : "MARKET"} BUY ${symbol}: $${riskAmount.toFixed(2)} @ $${fillPrice.toFixed(2)}`,
    );
...
async function executeExit(symbol: string, qty: number, reason: string): Promise<void> {
  const ticker = await fetchTicker(symbol);
  const fee =
    config.strategy.useLimitOrders && reason === "take-profit"
      ? config.strategy.makerFeePct
      : config.strategy.takerFeePct; // stop-loss always market (urgent)

  if (config.bot.dryRun) {
    log.trade(
      `[DRY RUN] SELL ${symbol}: ${qty.toFixed(6)} @ $${ticker.price.toFixed(2)} (reason: ${reason}, fee: ${(fee * 100).toFixed(2)}%)`,
    );
