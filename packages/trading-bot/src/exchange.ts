...
export async function createMarketBuy(symbol: string, usdAmount: number): Promise<string> {
  ...
  log.trade(`MARKET BUY ${qty.toFixed(6)} ${symbol} @ ~$${ticker.price} (cost: $${usdAmount.toFixed(2)})`)
...
export async function createLimitBuy(symbol: string, usdAmount: number, offsetPct: number): Promise<string> {
  const ex = getExchange()
  const ticker = await fetchTicker(symbol)
  const limitPrice = ticker.ask * (1 - offsetPct)
  const qty = usdAmount / limitPrice

  log.trade(`LIMIT BUY ${qty.toFixed(6)} ${symbol} @ $${limitPrice.toFixed(2)} (ask: $${ticker.ask.toFixed(2)})`)
...
export async function createLimitSell(symbol: string, qty: number, offsetPct: number): Promise<string> {
  const ex = getExchange()
  const ticker = await fetchTicker(symbol)
  const limitPrice = ticker.bid * (1 + offsetPct)

  log.trade(`LIMIT SELL ${qty.toFixed(6)} ${symbol} @ $${limitPrice.toFixed(2)} (bid: $${ticker.bid.toFixed(2)})`)
