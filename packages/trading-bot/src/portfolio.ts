...
export function closePosition(symbol: string, exitPrice: number) {
  ...
  log.trade(`${emoji} Closed ${symbol}: ${pnlPct.toFixed(2)}% PnL (entry $${pos.entryPrice} → exit $${exitPrice})`)
}
...
export async function getPortfolioSummary() {
  try {
    ...
    const cryptoBalances = balances.filter(b => !stablecoins.includes(b.currency) && b.currency !== 'USD' && b.total > 0)
