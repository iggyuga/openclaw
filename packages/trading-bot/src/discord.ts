    ...
    embeds: [
      {
        ...
        fields: [
          ...
          {
            name: "EMA Fast/Slow",
            value: `${emaFast.toFixed(2)} / ${emaSlow.toFixed(2)}`,
            inline: true,
          },
        ...
        footer: {
          text: config.bot.dryRun ? "DRY RUN — no real order placed" : "Live order placed",
        },
      },
    ],
  });
...
export async function notifySentiment(sentiment: SentimentSnapshot): Promise<void> {
  ...
  const fgEmoji =
    fearGreed.value < 30 ? "😱" : fearGreed.value < 50 ? "😟" : fearGreed.value < 70 ? "😐" : "🤑";
